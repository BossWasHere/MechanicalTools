...
public class EntityTeleporterCommand {

	public static void run(MechMain plugin, CommandSender sender, String[] args) {
		if (args.length < 1) {
			if (sender instanceof Player) {
				Player player = (Player) sender;
				if (player.hasPermission(plugin.permissions.entityTeleporter_command)) {
			...
			if (sender.hasPermission(plugin.permissions.entityTeleporter_commandOther)) {
				String t = args[0];
				Player target = null;
