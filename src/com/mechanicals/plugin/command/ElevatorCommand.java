...
public class ElevatorCommand extends RegisteredCommand {
	...
	public void run(CommandSender sender, String[] args) {
		if (args.length < 1) {
			if (sender instanceof Player) {
				...
				if (player.hasPermission(plugin.permissions.elevator_command)) {
					plugin.elevator.giveBlockItem(player);
				} else {
			...
			if (sender.hasPermission(plugin.permissions.elevator_commandOther)) {
				Player target = EntityUtils.getPlayer(args[0]);
				if (target == null) {
					sender.sendMessage(plugin.texts.playerNotFound(args[0]));
				} else {
					plugin.elevator.giveBlockItem(target);
