...
public class EntityTeleporter extends BaseMechanicalBlock {
	
	public EntityTeleporter(MechMain plugin) {
		super(plugin);
	}
	...
	public void blockPlaceEvent(BlockPlaceEvent event) {
		...
		if (!plugin.entityTeleporterEnabled) {
			player.sendMessage(plugin.texts.notEnabled);
			event.setCancelled(true);
			return;
		}
		if (player.hasPermission(plugin.permissions.entityTeleporter_place)) {
	...
	public MechanicalBlocks getMechBlock() {
		return MechanicalBlocks.ENTITY_TELEPORTER;
	}
	...
	public void blockBreakEvent(BlockBreakEvent event) {
		...
		if (!event.getPlayer().hasPermission(plugin.permissions.entityTeleporter_break)) {
					...
					if (plugin.placed.getInt(key + ".y") == event.getBlock().getY()) {
						if (plugin.placed.getInt(key + ".z") == event.getBlock().getZ()) {
							if (!plugin.placed.getString(key + ".player").equalsIgnoreCase(event.getPlayer().getUniqueId().toString())) {
								if (!event.getPlayer().hasPermission(plugin.permissions.entityTeleporter_breakOther)) {
