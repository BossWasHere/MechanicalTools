...
public class PlantFarmer extends BaseMechanicalBlock {
	...
	public void blockPlaceEvent(BlockPlaceEvent event) {
		...
		if (!plugin.plantFarmerEnabled) {
			player.sendMessage(plugin.texts.notEnabled);
			event.setCancelled(true);
			return;
		}
		if (player.hasPermission(plugin.permissions.plantFarmer_place)) {
	...
	public MechanicalBlocks getMechBlock() {
		return MechanicalBlocks.PLANT_FARMER;
	}

	@Override
	public void blockBreakEvent(BlockBreakEvent event) {
		if (!plugin.animalGrowthEnabled) {
