...
public class BlockEvent implements Listener {
	...
	public void blockPlaceEvent(BlockPlaceEvent event) {
		...
		} else if (MechMain.plugin.miner.matchesMeta(blockPlaced)) {
			MechMain.plugin.miner.blockPlaceEvent(event);
	...
	public void blockBreakEvent(BlockBreakEvent event) {
		...
		} else if (event.getBlock().getType() == Material.IRON_BLOCK || event.getBlock().getType() == Material.IRON_BLOCK) {
			for (String key : MechMain.plugin.placed.getKeys(false)) {
				if (MechMain.plugin.placed.getString(key + ".id", "").equals(MechanicalBlocks.MINER.getId())) {
					MechMain.plugin.miner.blockBreakEvent(event);
				}
