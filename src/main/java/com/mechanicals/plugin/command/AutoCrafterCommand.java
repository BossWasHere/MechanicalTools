...
public class AutoCrafterCommand extends RegisteredCommand {

	public AutoCrafterCommand() {
		super();
	}
	...
	public void run(CommandSender sender, String[] args) {
		if (args.length < 1) {
			if (sender instanceof Player) {
				...
				if (player.hasPermission(plugin.autoCrafter.commandPerm())) {
					plugin.autoCrafter.giveBlockItem(player);
				} else {
			...
			if (sender.hasPermission(plugin.autoCrafter.commandOtherPerm())) {
				Player target = EntityUtils.getPlayer(args[0]);
				if (target == null) {
					sender.sendMessage(plugin.texts.playerNotFound(args[0]));
				} else {
					plugin.autoCrafter.giveBlockItem(target);
