...
public class RecipeRegistry {
	...
	public static void registerRecipes(MechMain plugin) {
		...
		if (plugin.blockData.getBoolean("block.generator.crafting.enabled") && plugin.generatorEnabled) {
			Map<Character, ItemStack> items = ItemConfigurationEncoder.decodeStringItemParts(plugin.blockData.getString("block.generator.crafting.items"));
			ShapedRecipe recipe = getRecipeHolder(plugin, plugin.generator.getBlockItem(), plugin.generator.getMechBlock().getId());
			recipe.shape(plugin.blockData.getString("block.generator.crafting.a"), plugin.blockData.getString("block.generator.crafting.b"), plugin.blockData.getString("block.generator.crafting.c"));
			
			if (!register(plugin, recipe, items)) {
				plugin.logger.warning("The recipe for the block Generator failed to load. (Check the item shape has declared all chars)");
			}
		}
		if (plugin.blockData.getBoolean("block.miner.crafting.enabled") && plugin.minerEnabled) {
			Map<Character, ItemStack> items = ItemConfigurationEncoder.decodeStringItemParts(plugin.blockData.getString("block.miner.crafting.items"));
			ShapedRecipe recipe = getRecipeHolder(plugin, plugin.miner.getBlockItem(), plugin.miner.getMechBlock().getId());
			recipe.shape(plugin.blockData.getString("block.miner.crafting.a"), plugin.blockData.getString("block.miner.crafting.b"), plugin.blockData.getString("block.miner.crafting.c"));
			
			if (!register(plugin, recipe, items)) {
				plugin.logger.warning("The recipe for the block Miner failed to load. (Check the item shape has declared all chars)");
			}
		}
		if (plugin.blockData.getBoolean("block.autoCrafter.crafting.enabled") && plugin.autoCrafterEnabled) {
			Map<Character, ItemStack> items = ItemConfigurationEncoder.decodeStringItemParts(plugin.blockData.getString("block.autoCrafter.crafting.items"));
			ShapedRecipe recipe = getRecipeHolder(plugin, plugin.autoCrafter.getBlockItem(), plugin.autoCrafter.getMechBlock().getId());
			recipe.shape(plugin.blockData.getString("block.autoCrafter.crafting.a"), plugin.blockData.getString("block.autoCrafter.crafting.b"), plugin.blockData.getString("block.autoCrafter.crafting.c"));
			
			if (!register(plugin, recipe, items)) {
				plugin.logger.warning("The recipe for the block Auto Crafter failed to load. (Check the item shape has declared all chars)");
			}
		}
	...
	private static ShapedRecipe getRecipeHolder(MechMain plugin, ItemStack item, String namespace) {
		if (VersionMatcher.isAtOrHigherThan("1.12")) {
